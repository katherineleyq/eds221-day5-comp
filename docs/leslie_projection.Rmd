...
# The line below... creates a time vector from 0 to 8
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below... creates a matrix with column length = to fish year, and row length = time
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below...putting fish_year0 vector into first row of proj year
proj_year[1, ] <- fish_year0

# The for loop below...will populate the dot product of fish leslie and proj_year below row 1
#you can run a print(proj_year) to see it step by step
...
```{r}
# demo data for pivot longer
demo <- data.frame(year = c(1995, 2000, 2005),
                   cats = c(2, 4, 6),
                   dogs = c(10, 14, 12))

# ggplot for above (un-tidy) data
ggplot(data = demo) +
...
ggplot(data = demo_longer, aes(x=year, y = count, color = species)) +
  geom_line()
```
